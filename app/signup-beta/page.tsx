...
export default function SignUpBetaPage() {
  ...
  const handleGoogleSignUp = async () => {
    ...
    if (oauthError) {
      ...
      posthog.capture('organizer_signed_up', { method: 'google' });
    }
  };
    ...
    try {
      ...
      if (data.user) {
        if (posthog) {
          ...
          posthog.capture('organizer_signed_up', { method: 'email', email: data.user.email ?? undefined });
        }
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-sand px-4">
      <div className="w-full max-w-sm space-y-6">
        ...
        <div className="rounded-xl border border-charcoal/10 bg-surface p-6 shadow-soft space-y-4">
          {error && (
