...
- Name: organizer.smoke.ts >> Edit signup page >> description can be changed and persists after save
- Location: e2e/organizer.smoke.ts:331:7
...
Locator: locator('textarea[name="signupsmartly-event-description"]')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('textarea[name="signupsmartly-event-description"]')
...
```ts
  ...
  272 |   });
  273 | 
  274 |   test('unsaved changes modal cancel keeps user on edit page', async ({ page }) => {
